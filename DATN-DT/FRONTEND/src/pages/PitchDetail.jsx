...
export default function PitchDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [pitchRes, reviewRes] = await Promise.all([
          pitchAPI.getById(id),
          reviewAPI.getByPitch(id),
        ]);
        setPitch(pitchRes.data.data);
  ...
  const allSlots = pitch ? generateSlots(pitch.openTime, pitch.closeTime) : [];
  ...
  const handleReview = async (e) => {
    ...
    try {
      await reviewAPI.create({ pitchId: id, ...reviewForm });
            ...
            {tab === 'info' && (
              <div>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                      <span className="badge badge-primary">{TYPE_LABELS[pitch.type]}</span>
                      <span className="badge badge-success">{pitch.status === 'active' ? 'Đang hoạt động' : pitch.status}</span>
                    </div>
                  </div>
                  ...
                  <div className="card-body">
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                      {[
                        ...
                        { icon: '🏙️', label: 'Thành phố', value: `${pitch.district || ''} ${pitch.city || ''}`.trim() || '—' },
                        { icon: '🕐', label: 'Giờ mở cửa', value: pitch.openTime },
                        { icon: '🕙', label: 'Giờ đóng cửa', value: pitch.closeTime },
                        { icon: '⭐', label: 'Đánh giá', value: `${(pitch.averageRating || 0).toFixed(1)} / 5 (${pitch.totalReviews || 0} đánh giá)` },
                      ].map(({ icon, label, value }) => (
                        <div key={label}>
                          <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: 3 }}>{icon} {label}</div>
                ...
                {pitch.description && (
                  <div className="card" style={{ marginBottom: 20 }}>
                    <div className="card-body">
                      <h3 style={{ marginBottom: 10 }}>Mô tả</h3>
                ...
                {pitch.amenities?.length > 0 && (
                  <div className="card">
                    <div className="card-body">
                      <h3 style={{ marginBottom: 14 }}>Tiện ích</h3>
                      ...
                      <div key={r._id} className="card">
                        <div className="card-body">
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                            <div style={{ fontWeight: 700 }}>{r.user?.name || 'Ẩn danh'}</div>
          ...
          <div style={{ position: 'sticky', top: 88 }}>
            <div className="card">
              <div className="card-header">
                <h3 style={{ fontWeight: 700 }}>📅 Đặt sân</h3>
              </div>
              <div className="card-body">
                {/* Date Picker */}
                ...
                <div className="form-group">
                  <label className="form-label" style={{ marginBottom: 10 }}>
                    Khung giờ {selectedSlot && <span style={{ color: 'var(--primary-light)' }}>— đã chọn: {selectedSlot.label}</span>}
                  </label>
                  {loadingSlots ? (
                ...
                {selectedSlot && (
                  <div className="form-group">
                    ...
                    <textarea
                      className="form-control"
                      rows={2}
                      placeholder="Thông tin thêm cho chủ sân..."
                      value={note}
                    ...
                    {[1, 2, 3, 4, 5].map((s) => (
                      <button
                        ...
                        style={{
                          ...
                          opacity: s <= reviewForm.rating ? 1 : 0.3, transition: 'var(--transition)'
                        }}
